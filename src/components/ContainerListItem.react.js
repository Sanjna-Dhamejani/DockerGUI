...
var ContainerListItem = React.createClass({
  ...
  render: function() {
    ...
    var imageName = (
      <OverlayTrigger
        placement="bottom"
        overlay={<Tooltip>{container.Config.Image}</Tooltip>}
      >
        <span>{repo}</span>
      </OverlayTrigger>
    );
    ...
    if (container.State.Downloading) {
      state = (
        <OverlayTrigger
          placement="bottom"
          overlay={<Tooltip>Downloading</Tooltip>}
        >
          <div className="state state-downloading">
            <div style={style} className="downloading-arrow" />
          </div>
        </OverlayTrigger>
      );
      ...
      state = (
        <OverlayTrigger placement="bottom" overlay={<Tooltip>Running</Tooltip>}>
          <div className="state state-running">
            <div style={style} className="runningwave" />
          </div>
        </OverlayTrigger>
      );
      ...
      state = (
        <OverlayTrigger
          placement="bottom"
          overlay={<Tooltip>Restarting</Tooltip>}
        >
          <div className="state state-restarting" style={style} />
        </OverlayTrigger>
      );
    } else if (container.State.Paused) {
      state = (
        <OverlayTrigger placement="bottom" overlay={<Tooltip>Paused</Tooltip>}>
          <div className="state state-paused" />
        </OverlayTrigger>
      );
      ...
      <Router.Link to="container" params={{ name: container.Name }}>
        ...
        >
          ...
          <div className="info">
            ...
            <div className="image">{imageName}</div>
          </div>
          <div className="action">
            <span
              className={
                container.Favorite ? "btn circular favorite" : "btn circular"
              }
              onClick={this.toggleFavoriteContainer}
            >
              <span className="icon icon-favorite" />
            </span>
            <span className="btn circular" onClick={this.handleDeleteContainer}>
