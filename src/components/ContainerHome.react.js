...
var ContainerHome = React.createClass({
  ...
  showFolders: function() {
    return (
      ...
      this.props.container.Mounts.length > 0 &&
    ...
    if (this.props.container.Error) {
      ...
      if (!error) {
        ...
        if (error.indexOf("ETIMEDOUT") !== -1) {
          error =
            'Timeout error - Try and restart your VM by running: \n"docker-machine restart default" in a terminal';
        }
        if (error.indexOf("ECONNREFUSED") !== -1) {
          error =
            'Is your VM up and running? Check that "docker ps" works in a terminal.';
        }
      }
      body = (
        <div className="details-progress error">
          <h2>We are sorry. There seems to be an error:</h2>
          {error.split("\n").map(i => {
      ...
      if (this.props.container.Progress) {
        ...
        for (let i = 0; i < this.props.container.Progress.amount; i++) {
          values.push(
            Math.round(this.props.container.Progress.progress[i].value)
          );
      ...
      if (logWidget && !webWidget && !folderWidget) {
        body = (
          <div className="details-panel home">
            <div className="content">
              <div className="full">{logWidget}</div>
            </div>
          </div>
        );
      } else {
        body = (
          <div className="details-panel home">
            <div className="content">
              <div className="left">{logWidget}</div>
              <div className="right">
                {webWidget}
                {folderWidget}
              </div>
