...
var ContainerSettingsPorts = React.createClass({
  ...
  checkPort: function(ports, port, key) {
    ...
    const otherContainers = _.filter(
      _.values(containerStore.getState().containers),
      c => c.Name !== this.props.container.Name
    );
    const otherPorts = _.flatten(
      otherContainers.map(container => {
        try {
          return _.values(container.NetworkSettings.Ports).map(hosts =>
            hosts.map(host => {
    ...
    if (!port.match(/^[0-9]+$/g)) {
      ...
      ports[key].error =
        "Ports 22 and 2376 are reserved ports for Docker GUI/Docker.";
    }
  },
  ...
  handleSave: function() {
    ...
    let hostConfig = _.extend(this.props.container.HostConfig, {
      PortBindings: portBindings,
      Hostname: this.state.hostname
    });
    let config = _.extend(this.props.container.Config, {
      Hostname: this.state.hostname
    });
  ...
  render: function() {
    ...
    var ports = _.map(_.pairs(this.state.ports), pair => {
      ...
      let ipLink =
        ...
        !this.props.container.State.Paused &&
      ...
      if (key === "") {
        ...
        if (this.isInitialPort(key, this.state.initialPorts)) {
          ...
          icon = (
            <td>
              <a
                disabled={isUpdating}
                onClick={this.handleRemovePort.bind(this, key)}
        ...
        portKey = (
          <input
            id={"portKey" + key}
            type="text"
            onChange={this.handleChangePortKey.bind(this, key)}
            disabled={isUpdating}
            defaultValue={key}
          />
        );
      ...
      return (
        <tr key={key}>
          ...
          <td>
            ...
            >
              <MenuItem
                onSelect={this.handleChangePortType.bind(this, key, "tcp")}
                key={key + "-tcp"}
              >
                TCP
              </MenuItem>
              <MenuItem
                onSelect={this.handleChangePortType.bind(this, key, "udp")}
                key={key + "-udp"}
              >
                UDP
              </MenuItem>
