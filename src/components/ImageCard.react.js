...
var ImageCard = React.createClass({
  ...
  handleClick: function() {
    ...
    let repo =
      this.props.image.namespace === "library" ||
      this.props.image.namespace === "local"
        ? this.props.image.name
        : this.props.image.namespace + "/" + this.props.image.name;

    containerActions.run(
      name,
      repo,
  ...
  handleDeleteImgClick: function(image) {
    if (this.state.chosenTag && !this.props.image.inUse) {
      imageActions.destroy(
        image.RepoTags[0].split(":")[0] + ":" + this.state.chosenTag
      );
    }
  },
  ...
  render: function() {
    ...
    if (this.state.loading) {
      ...
      var tagDisplay = this.state.tags
        .filter(tag => tag.name.includes(this.state.searchTag))
        .map(tag => {
          let t = "";
          if (tag.name) {
            t = tag.name;
          ...
          if (t === this.state.chosenTag) {
            return (
              <div
                ...
                key={key}
                onClick={this.handleTagClick.bind(this, t)}
              >
                {t}
              </div>
            );
      ...
      tags = <div className="item-list tag-list">{tagDisplay}</div>;
    }
      ...
      if (networkName === this.state.defaultNetwork) {
        return (
          <div
            className="item active"
            key={networkName}
            onClick={this.handleNetworkClick.bind(this, networkName)}
          >
            {networkName}
          </div>
        );
    ...
    let networks = (
      <div className="item-list network-list">{networkDisplay}</div>
    );

    var badge = null;
    if (this.props.image.namespace === "library") {
      badge = <span className="icon icon-badge-official" />;
    } else if (this.props.image.is_private) {
      badge = <span className="icon icon-badge-private" />;
    }
    ...
    if (this.props.image.is_local) {
      ...
      overlay = (
        <div className="overlay menu-overlay">
          <div
            className="menu-item"
            onClick={this.handleTagOverlayClick.bind(
              this,
              this.props.image.name
            )}
          >
            ...
            <span className="text">
              SELECTED TAG:{" "}
              <span className="selected-item">{this.state.chosenTag}</span>
            </span>
          </div>
          <div
            className="remove"
      ...
      let favCount =
        this.props.image.star_count < 1000
          ? numeral(this.props.image.star_count).value()
          : numeral(this.props.image.star_count)
              .format("0.0a")
              .toUpperCase();
      let pullCount =
        this.props.image.pull_count < 1000
          ? numeral(this.props.image.pull_count).value()
          : numeral(this.props.image.pull_count)
              .format("0a")
              .toUpperCase();
      create = (
        <div className="actions">
      ...
      overlay = (
        <div className="overlay menu-overlay">
          ...
          <div
            className="menu-item"
            onClick={this.handleNetworkOverlayClick.bind(
              this,
              this.props.image.name
            )}
          >
            ...
            <span className="text">
              DEFAULT NETWORK:{" "}
              <span className="selected-item">{this.state.defaultNetwork}</span>
            </span>
          </div>
      ...
      <div className="image-item">
        ...
        <div className="overlay item-overlay tag-overlay">
          ...
          <div className="close-overlay" onClick={this.handleCloseTagOverlay}>
            <a className="btn btn-action circular">
              <span className="icon icon-delete" />
            </a>
          </div>
        </div>
